...
filecontent = fileread(fullfile('MFCRaw1.gcode'));
...
coordinates = str2double(cellfun(@(x) x(2:end), coordinates, 'UniformOutput', false));
 coordinates = coordinates(4:length(coordinates)-15,:);

z=0;
 for i=1:length(coordinates)
    if isnan(coordinates(i,3))==0
        z=coordinates(i,3);
    else coordinates(i,3)=z;
    end
 end

ix=ismissing(coordinates);
coordinates=coordinates(~any(ix,2),:);
...
rotAngle2=deg2rad(30-angc);
rotAngle3=deg2rad(0-angc);
rotAngle4=deg2rad(60-angc);
% factor_datascrub=0;
...
writetable(T_dimensional,'MFCRaw1GCodeWidthLength')
...
writetable(T_geometrical,'MFC3Raw1GCodeStandardDeviation')
